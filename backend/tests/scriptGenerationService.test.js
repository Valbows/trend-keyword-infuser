...
describe('Script Generation Service', () => {
  const mockTrends = [
    { source: 'Google Trends', keyword: 'AI in education', link: 'http://example.com/ai-edu', pubDate: '2024-05-01', snippet: 'AI is transforming education.' },
    { source: 'YouTube', keyword: 'Top 5 AI tools', link: 'http://youtube.com/top-ai', pubDate: '2024-05-02', snippet: 'Discover the best AI tools.' },
  ];
  ...
  beforeEach(() => {
    ...
    if (mocks.GoogleGenerativeAI.mockClear) { // Check if it's a jest mock function
  ...
  it('should generate a script successfully when API key is provided', async () => {
    ...
    require('@google/generative-ai').__mockGenerateContent.mockResolvedValue({ response: { text: () => mockScript } });

    const script = await generateScript(topic, mockTrends);

    expect(script).toBe(mockScript);
    expect(require('@google/generative-ai').__mockGetGenerativeModel).toHaveBeenCalledWith({ model: 'gemini-1.5-flash' });
    expect(require('@google/generative-ai').__mockGenerateContent).toHaveBeenCalledTimes(1);
  ...
  it('should throw an error if GEMINI_API_KEY is not set', async () => {
    ...
    await expect(generateScript(topic, mockTrends)).rejects.toThrow('GEMINI_API_KEY is not set.');
  });

  it('should throw an error if Gemini API call fails', async () => {
    require('@google/generative-ai').__mockGenerateContent.mockRejectedValue(new Error('Gemini API error'));
    await expect(generateScript(topic, mockTrends)).rejects.toThrow('Failed to generate script from Gemini API: Gemini API error');
  });

  it('should throw an error if Gemini API response is not as expected', async () => {
    require('@google/generative-ai').__mockGenerateContent.mockResolvedValue({ response: { text: () => undefined } }); // Simulate unexpected response
    await expect(generateScript(topic, mockTrends)).rejects.toThrow('Failed to get valid script content from Gemini API response.');
  });
...
Ensure the script flows naturally and is ready for text-to-speech conversion.`;
    ...
    const actualPrompt = require('@google/generative-ai').__mockGenerateContent.mock.calls[0][0];
