...
async function handleRecordEngagement(req, res) {
  ...
  logger.info(
    `[ScriptController] Received request to record engagement for scriptId: ${scriptId}`
  );

  if (!videoUrl) {
    return res
      .status(400)
      .json({ success: false, message: 'The videoUrl field is required.' });
  }

  try {
    const { data, error } = await engagementRecordingService.recordEngagement(
      scriptId,
      videoUrl
    );
    ...
    if (error) {
      logger.error(
        `[ScriptController] Error recording engagement for scriptId ${scriptId}: ${error.message}`
      );
      return res
    ...
    logger.info(
      `[ScriptController] Successfully recorded engagement for scriptId: ${scriptId}`
    );
...
const handleGenerateScript = async (req, res) => {
  ...
  if (!topic) {
    return res
      .status(400)
      .json({ success: false, message: 'Missing required field: topic' });
  }
  ...
  try {
    ...
    logger.error(
      `[ScriptController] Error generating script for topic "${topic}":`,
      error
    );
    const statusCode = error.status || 500;
    const message =
      error.message ||
      'Failed to generate script due to an internal server error.';
...
const handleGetAllScripts = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: 'Failed to retrieve scripts.' });
  }
};
...
const handleGetScriptById = async (req, res) => {
  ...
  try {
    ...
    logger.error(
      `[ScriptController] Error fetching script with ID ${id}:`,
      error
    );
    const statusCode = error.status || 500;
...
const handleModifyScript = async (req, res) => {
  ...
  try {
    ...
    logger.error(
      'Error in scriptController handling script modification:',
      error
    );
...
const handleUpdateScriptContent = async (req, res) => {
  ...
  if (!id) {
    return res.status(400).json({
      success: false,
      message: 'Script ID is required in URL parameters.',
    });
  }
  ...
  try {
    const updatedScript = await scriptPersistenceService.updateScriptContent(
      id,
      content
    );
    ...
    logger.error(
      `[ScriptController] Error updating script with ID ${id}:`,
      error
    );
    const statusCode = error.status || 500;
    const message =
      error.message ||
      'An unexpected error occurred while updating the script.';
    res.status(statusCode).json({ success: false, message });
  }
};
