...
const handleGenerateScript = async (req, res) => {
  ...
  if (cachedData) {
    console.log(
      `[scriptController.handleGenerateScript] Serving from cache for key: ${cacheKey}`
    );
  ...
  try {
    console.log(
      `Controller: Received request to generate script for topic: ${topic}`
    );
    const scriptText =
    ...
    try {
      ...
      if (dbError) {
        ...
        console.log(
          `Script ID ${scriptId} successfully retrieved for response.`
        );
      } else {
        console.error(
          'Failed to retrieve script ID from Supabase after insert, or dbData is unexpected:',
          dbData
        );
      }
    } catch (e) {
    ...
    console.log(
      `[scriptController.handleGenerateScript] Caching new data for key: ${cacheKey}`
    );

    res.status(200).json(responseData);
  } catch (error) {
    console.error(
      'Error in scriptController handling script generation:',
      error.message
    );
  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === 'PGRST116') {
        // PGRST116 is the code for .single() finding no rows
...
const handleModifyScript = async (req, res) => {
  ...
  try {
    console.log(
      `Controller: Received request to modify script with ${selectedKeywords.length} keywords for user ID: ${userId}.`
    );
    ...
    console.error(
      'Raw error in scriptController handling script modification:',
      error
    );
    ...
    console.error(
      `Error in scriptController handling script modification (Processed): Message: "${errorMessageString}", Stack: ${errorStack}`
    );
...
const handleUpdateScriptContent = async (req, res) => {
  ...
  if (!id) {
    return res
      .status(400)
      .json({ error: 'Script ID is required in URL parameters.' });
  }
  ...
  if (typeof content !== 'string') {
    return res
      .status(400)
      .json({ error: 'Invalid request body: content must be a string.' });
  }
  ...
  try {
    ...
    if (error) {
      ...
      return res
        .status(500)
        .json({ error: 'Failed to update script due to a database error.' });
    }
    ...
    if (!data || data.length === 0) {
      ...
      return res
        .status(404)
        .json({ error: 'Script not found or no changes made.' });
    }
