try {
  if (process.env.NODE_ENV !== 'test') {
    ...
    if (dotenvResult.error) {
      ...
      console.error(
        'dotenv.config() error in scriptGenerationService (non-critical, will proceed to API key check):',
        dotenvResult.error
      );
    }
  }
  ...
  console.error(
    'CRITICAL error during dotenv setup in scriptGenerationService:',
    e
  );
  // Throw a new error to ensure the module loading fails clearly if dotenv setup is catastrophic.
  throw new Error(
    `Failed to initialize dotenv configuration in scriptGenerationService. Module cannot load. Original error: ${e.message}`
  );
}
...
`;
  ...
  try {
    ...
    if (
      ...
      let tempDetail =
        'Failed to extract specific details from complex error object.'; // Default for this complex path
      try {
