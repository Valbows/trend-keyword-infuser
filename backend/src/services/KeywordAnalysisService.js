...
async function getRelevanceForKeywords(keywordsArray, contextTopic) {
  if (!model) {
    console.warn(
      'KeywordAnalysisService.getRelevanceForKeywords: Gemini model not initialized (API Key likely missing). Skipping AI relevance.'
    );
  ...
  if (
    ...
    console.warn(
      'KeywordAnalysisService.getRelevanceForKeywords: Context topic is invalid. Skipping AI relevance.'
    );
    return keywordsArray.map((kw) => ({ ...kw, aiRelevance: null }));
  }
  ...
  try {
    ...
    if (!response || typeof response.text !== 'function') {
      console.error(
        'KeywordAnalysisService: Invalid response structure from Gemini API.'
      );
      return keywordsArray.map((kw) => ({
        ...kw,
        aiRelevance: { error: 'Invalid AI response structure' },
      }));
    }
    ...
    try {
      ...
      console.error(
        'KeywordAnalysisService: Failed to parse JSON response from Gemini:',
        parseError
      );
      console.error(
        'KeywordAnalysisService: Gemini raw response text:',
        responseText
      );
      return keywordsArray.map((kw) => ({
        ...kw,
        aiRelevance: { error: 'Failed to parse AI response' },
      }));
    }

    if (!Array.isArray(aiResults)) {
      console.error(
        'KeywordAnalysisService: Gemini response is not a JSON array. Raw text:',
        responseText
      );
      return keywordsArray.map((kw) => ({
        ...kw,
        aiRelevance: { error: 'AI response not an array' },
      }));
    }
    ...
    aiResults.forEach((res) => {
      if (
        ...
        console.warn(
          'KeywordAnalysisService: Malformed item in AI results array:',
          res
        );
      }
    });
    ...
    const augmentedKeywords = keywordsArray.map((originalKeywordObj) => {
      ...
      return {
        ...originalKeywordObj,
        aiRelevance: aiData
          ? { score: aiData.score, justification: aiData.justification }
          : { error: 'Keyword not found in AI results or AI result malformed' },
      };
    });

    console.info(
      `KeywordAnalysisService: Successfully processed AI relevance for ${augmentedKeywords.filter((kw) => kw.aiRelevance && !kw.aiRelevance.error).length} out of ${keywordsArray.length} keywords.`
    );
    return augmentedKeywords;
  } catch (error) {
    console.error(
      'KeywordAnalysisService: Error calling Gemini API for keyword relevance:',
      error
    );
    const errorMessage = error instanceof Error ? error.message : String(error);
    return keywordsArray.map((kw) => ({
      ...kw,
      aiRelevance: { error: `AI API call failed: ${errorMessage}` },
    }));
  }
}
