...
class TrendDiscoveryService {
  ...
  async _extractKeywordsFromVideos(videos, timeframe, topic) { // Added topic parameter for AI context
    ...
    const videoDates = videos.map(v => new Date(v.snippet.publishedAt)).filter(d => !isNaN(d.getTime()));
    const minDate = videoDates.length > 0 ? Math.min(...videoDates.map(d => d.getTime())) : Date.now();
    const maxDate = videoDates.length > 0 ? Math.max(...videoDates.map(d => d.getTime())) : Date.now();
    ...
    for (const video of videos) {
      ...
      for (const token of tokens) {
        if (token.length >= MIN_KEYWORD_LENGTH && !STOP_WORDS.includes(token)) {
          ...
          const recencyScore = videoDates.length > 0 && !isNaN(currentVideoDate.getTime()) ? (currentVideoDate.getTime() - minDate) / dateRange : 0.5; // Default to 0.5 if date is invalid
    ...
    for (const [keyword, data] of keywordMap.entries()) {
      ...
      const averageRecency = data.count > 0 ? data.weighted_recency_score_sum / data.count : 0.5;
      const engagementScore = data.count * data.source_videos.length * (0.5 + averageRecency); // Base of 0.5 + (0 to 1), so multiplier is 0.5 to 1.5
    ...
    const rawExtractedKeywords = extractedKeywords.slice(0, MAX_KEYWORDS_TO_RETURN);
    logger.info(`[TrendDiscoveryService] Extracted ${rawExtractedKeywords.length} raw keywords for timeframe: ${timeframe}.`);

    if (rawExtractedKeywords.length > 0 && topic) {
      logger.info(`[TrendDiscoveryService] Augmenting ${rawExtractedKeywords.length} keywords with AI relevance for topic: "${topic}"`);
      try {
        const keywordsWithAiRelevance = await getRelevanceForKeywords(rawExtractedKeywords, topic);
        logger.info(`[TrendDiscoveryService] Successfully augmented ${keywordsWithAiRelevance.filter(kw => kw.aiRelevance && !kw.aiRelevance.error).length} out of ${rawExtractedKeywords.length} keywords with AI relevance.`);
        return keywordsWithAiRelevance;
      } catch (aiError) {
        logger.error(`[TrendDiscoveryService] Error during AI relevance augmentation: ${aiError.message}. Returning raw keywords.`);
        // Return raw keywords but mark them as having failed AI augmentation
        return rawExtractedKeywords.map(kw => ({ ...kw, aiRelevance: { error: `AI augmentation failed: ${aiError.message}` } }));
      }
    } else {
      if (!topic) {
        logger.warn(`[TrendDiscoveryService] Topic not provided to _extractKeywordsFromVideos. Skipping AI relevance for ${rawExtractedKeywords.length} keywords.`);
      }
      logger.info(`[TrendDiscoveryService] No keywords to augment or topic missing. Returning ${rawExtractedKeywords.length} raw keywords without AI relevance.`);
  ...
  async getYouTubeKeywordsByTopicAndTimeframe(topic, timeframe = 'any', publishedAfterISO = null, publishedBeforeISO = null) {
    ...
    if (publishedAfterISO) {
      try {
        ...
        logger.debug(`[TrendDiscoveryService] Applying custom publishedAfter filter: ${params.publishedAfter}`);
      } catch (e) {
        logger.warn(`[TrendDiscoveryService] Invalid publishedAfterISO format: ${publishedAfterISO}. Ignoring.`);
      }
    }
    if (publishedBeforeISO) {
      try {
        ...
        effectiveTimeframeLabel = 'custom'; // Indicate a custom range was used
        logger.debug(`[TrendDiscoveryService] Applying custom publishedBefore filter: ${params.publishedBefore}`);
      } catch (e) {
        logger.warn(`[TrendDiscoveryService] Invalid publishedBeforeISO format: ${publishedBeforeISO}. Ignoring.`);
      }
    }

    // Only apply fixed timeframe logic if custom dates weren't provided or were invalid for publishedAfter
    if (timeframe !== 'any' && !params.publishedAfter && !params.publishedBefore) {
      const now = new Date();
    ...
    try {
      ...
      const finalKeywords = await this._extractKeywordsFromVideos(items, effectiveTimeframeLabel, topic);
