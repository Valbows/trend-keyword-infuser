...
jest.mock(
  '../src/components/ScriptEditor',
  () => (props: MockScriptEditorProps) => {
    ...
    return (
      <div>
        ...
        <button onClick={() => props.onSave('edited content from mock')}>
          Save Mock
        </button>
...
const mockExistingScripts = [
  {
    id: 'existing-1',
    topic: 'Existing Topic 1',
    generated_script: 'Content for existing script 1',
    created_at: new Date().toISOString(),
  },
  {
    id: 'existing-2',
    topic: 'Existing Topic 2',
    generated_script: 'Content for existing script 2',
    created_at: new Date().toISOString(),
  },
];
...
describe('ScriptWorkflowOrchestrator - G.O.A.T. C.O.D.E.X. B.O.T. Supreme Validation', () => {
  ...
  test('G.O.A.T. Initial Render: Clairvoyantly displays main UI elements and fetches existing scripts', async () => {
    ...
    expect(
      screen.getByRole('heading', { name: /Load Existing Script/i })
    ).toBeInTheDocument();
    ...
    expect(mockScriptEditor).not.toHaveBeenCalledWith(
      expect.objectContaining({ initialScriptContent: expect.any(String) })
    );
  });
  ...
  test('C.O.D.E.X. Optimized New Script Generation: Handles topic input, generation, and editor display', async () => {
    ...
    const topicInput = screen.getByPlaceholderText(
      /Enter a topic \(e.g., Future of AI\)/i
    );
  ...
  test('C.O.D.E.X. Elegant Save (New Script): Saves newly generated and edited script', async () => {
    const localMockGeneratedScript = {
      scriptId: 'temp-new-id-789',
      script: 'Freshly generated script.',
    };
    (fetch as jest.Mock)
      .mockResolvedValueOnce({ ok: true, json: async () => [] }) // Initial load
      .mockResolvedValueOnce({
        ok: true,
        json: async () => localMockGeneratedScript,
      }) // Generation
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          ...mockUpdatedScript,
          id: localMockGeneratedScript.scriptId,
          generated_script: 'edited content from mock',
        }),
      }); // Save
    ...
    // Generate new script
    fireEvent.change(
      screen.getByPlaceholderText(/Enter a topic \(e.g., Future of AI\)/i),
      { target: { value: 'Ephemeral Topic' } }
    );
    fireEvent.click(screen.getByRole('button', { name: /Generate Script/i }));
    await waitFor(() =>
      expect(mockScriptEditor).toHaveBeenCalledWith(
        expect.objectContaining({
          initialScriptContent: localMockGeneratedScript.script,
        })
      )
    );
    ...
    const mockEditorSaveButton = screen.getByRole('button', {
      name: /Save Mock/i,
    });
  ...
  test('G.O.A.T. Altruistic Error Handling: Displays error for script generation failure', async () => {
    ...
    fireEvent.change(
      screen.getByPlaceholderText(/Enter a topic \(e.g., Future of AI\)/i),
      { target: { value: 'Error Topic' } }
    );
    fireEvent.click(screen.getByRole('button', { name: /Generate Script/i }));

    await waitFor(() => {
      expect(
        screen.getByText(/Generation Error: Generation Failed/i)
      ).toBeInTheDocument();
    });
  });
  ...
  test('G.O.A.T. Altruistic Error Handling: Displays error for fetching existing scripts failure', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Error loading scripts: Failed to fetch scripts/i)
      ).toBeInTheDocument();
    });
  });
  ...
  test('G.O.A.T. Altruistic Error Handling: Displays error in editor for save failure', async () => {
    (fetch as jest.Mock)
      ...
      .mockResolvedValueOnce({
        ok: false,
        status: 500,
        json: async () => ({ error: 'Save Failed Miserably' }),
      }); // Save fails
  ...
  test('C.O.D.E.X. Clairvoyant Input Validation: Prevents generation with empty topic and shows error', async () => {
    (fetch as jest.Mock).mockResolvedValueOnce({
      // For initial fetching existing scripts
      ok: true,
    ...
    expect(
      await screen.findByText(/Generation Error: Topic cannot be empty./i)
    ).toBeInTheDocument();
  ...
  test('C.O.D.E.X. Clairvoyant API Handling (Generation): Displays error for malformed successful response', async () => {
    (fetch as jest.Mock)
      .mockResolvedValueOnce({ ok: true, json: async () => [] }) // Initial load
      .mockResolvedValueOnce({
        // Generation API returns 200 OK but malformed body
        ok: true,
    ...
    fireEvent.change(topicInput, {
      target: { value: 'Malformed Response Test' },
    });
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(
          /Generation Error: Invalid response from script generation: ID or content missing./i
        )
      ).toBeInTheDocument();
    });
  ...
  test('C.O.D.E.X. Clairvoyant API Handling (Generation): Uses input topic for title if API response lacks topic', async () => {
    ...
    (fetch as jest.Mock)
      .mockResolvedValueOnce({ ok: true, json: async () => [] }) // Initial load
      .mockResolvedValueOnce({
        // Generation API returns 200 OK, valid script/ID, but no topic
        ok: true,
        json: async () => ({
          scriptId: 'script-no-topic-123',
          script: 'Content for script missing topic in response.',
        }),
      } as Response);
  ...
  test('C.O.D.E.X. Comprehensive Empty State: Displays message when no existing scripts are found', async () => {
    ...
    expect(
      screen.queryByRole('button', { name: /Load for Editing/i })
    ).not.toBeInTheDocument();
  });
  ...
  test('C.O.D.E.X. Clairvoyant Edge Case (Save): Prevents save and shows error if no script ID is present', async () => {
    ...
    expect(
      screen.queryByText(/No script selected or ID missing to save./i)
    ).not.toBeInTheDocument();
