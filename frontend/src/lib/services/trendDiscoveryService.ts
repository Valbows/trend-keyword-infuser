...
const STOP_WORDS = new Set([
  'a', 'an', 'and', 'are', 'as', 'at', 'be', 'but', 'by', 'for', 'if', 'in', 'into', 'is', 'it', 'no', 'not', 'of', 'on', 'or', 'such', 'that', 'the', 'their', 'then', 'there', 'these', 'they', 'this', 'to', 'was', 'will', 'with', 'i', 'me', 'my', 'myself', 'we', 'our', 'ours', 'ourselves', 'you', 'your', 'yours', 'yourself', 'yourselves', 'he', 'him', 'his', 'himself', 'she', 'her', 'hers', 'herself', 'it', 'its', 'itself', 'they', 'them', 'their', 'theirs', 'themselves', 'what', 'which', 'who', 'whom', 'this', 'that', 'these', 'those', 'am', 'is', 'are', 'was', 'were', 'be', 'been', 'being', 'have', 'has', 'had', 'having', 'do', 'does', 'did', 'doing', 'a', 'an', 'the', 'and', 'but', 'if', 'or', 'because', 'as', 'until', 'while', 'of', 'at', 'by', 'for', 'with', 'about', 'against', 'between', 'into', 'through', 'during', 'before', 'after', 'above', 'below', 'to', 'from', 'up', 'down', 'in', 'out', 'on', 'off', 'over', 'under', 'again', 'further', 'then', 'once', 'here', 'there', 'when', 'where', 'why', 'how', 'all', 'any', 'both', 'each', 'few', 'more', 'most', 'other', 'some', 'such', 'no', 'nor', 'not', 'only', 'own', 'same', 'so', 'than', 'too', 'very', 's', 't', 'can', 'will', 'just', 'don', 'should', 'now', 'com', 'http', 'https', 'www', 'youtube', 'channel', 'video', 'videos', 'playlist', 'playlists', 'watch', 'new', 'official', 'music', 'trailer', 'episode', 'series', 'full', 'hd', 'live', 'stream', 'highlights', 'interview', 'podcast', 'review', 'tutorial', 'guide', 'how', 'best', 'top', 'update', 'news', 'daily', 'weekly', 'monthly', 'yearly', '2023', '2024', '2025', 'like', 'subscribe', 'comment', 'share', 'follow', 'description', 'link', 'check', 'get', 'free', 'buy', 'download', 'learn', 'discover', 'explore', 'join', 'visit', 'find', 'out', 'more', 'info', 'details', 'contact', 'us', 'today', 'latest'
]);
...
class TrendDiscoveryService {
    private async _extractKeywordsFromVideos(videos: youtube_v3.Schema$SearchResult[], topic: string): Promise<ExtractedKeyword[]> {
        ...
        const videoDates = videos.map(v => new Date(v.snippet?.publishedAt || 0)).filter(d => !isNaN(d.getTime()));
            ...
            for (const token of tokens) {
                if (token.length >= MIN_KEYWORD_LENGTH && !STOP_WORDS.has(token)) {
                    if (!keywordMap.has(token)) {
                        keywordMap.set(token, { count: 0, weighted_recency_score_sum: 0, source_videos: new Set() });
                    }
                    const keywordData = keywordMap.get(token)!;
                    const currentVideoDate = new Date(video.snippet.publishedAt!);
                    const recencyScore = !isNaN(currentVideoDate.getTime()) ? (currentVideoDate.getTime() - minDate) / dateRange : 0.5;
        ...
        return extractedKeywords.sort((a, b) => b.engagement_score - a.engagement_score).slice(0, 50); // Return top 50 keywords
    }

    public async findYouTubeKeywordTrends(topic: string, timeframe: string, publishedAfter?: string, publishedBefore?: string): Promise<YouTubeKeywordItem[]> {
        if (!youTubeDataService) {
            throw new Error('YouTubeDataService is not initialized. Check YOUTUBE_API_KEY.');
        }
