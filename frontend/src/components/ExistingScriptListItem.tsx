...
const ExistingScriptListItem: React.FC<ExistingScriptListItemProps> = ({
  ...
  const handleRecordEngagement = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to record engagement'
      );
    } finally {
      setIsLoading(false);
    }
    ...
    >
      <div className='flex justify-between items-start'>
        ...
        <div className='w-1/2 pl-4'>
          {script.engagement_rate !== null &&
          script.engagement_rate !== undefined ? (
            <div className='text-right'>
              <h4 className='text-lg font-semibold text-green-400'>
                Engagement Rate: {script.engagement_rate.toFixed(2)}%
              </h4>
              <p className='text-sm text-gray-300'>
                Views: {script.views?.toLocaleString()}
              </p>
              <p className='text-sm text-gray-300'>
                Likes: {script.likes?.toLocaleString()}
              </p>
              <p className='text-sm text-gray-300'>
                Comments: {script.comments?.toLocaleString()}
              </p>
              <p className='text-xs text-gray-500 mt-1'>
                Last updated:{' '}
                {script.engagement_retrieved_at
                  ? new Date(script.engagement_retrieved_at).toLocaleString()
                  : 'N/A'}
              </p>
            </div>
          ) : (
            <form
              onSubmit={handleRecordEngagement}
