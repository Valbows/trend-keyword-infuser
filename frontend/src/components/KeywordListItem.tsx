...
const KeywordListItem: React.FC<KeywordListItemProps> = ({ keywordItem }) => {
  ...
  return (
    ...
    >
      <h3 className='text-md text-sky-300 font-semibold'>
        {keywordItem.keyword}
      </h3>
      <p className='text-xs text-slate-400'>
        Engagement Score:{' '}
        <span className='font-medium text-sky-400'>
          {keywordItem.engagement_score}
        </span>
      </p>
      <p className='text-xs text-slate-400'>
        Source Videos:{' '}
        <span className='font-medium text-slate-300'>
          {keywordItem.source_video_count}
        </span>{' '}
        | Timeframe:{' '}
        <span className='font-medium text-slate-300'>
          {keywordItem.timeframe}
        </span>
      </p>
        ...
        <div className='mt-1 pt-1 border-t border-slate-600/50'>
          {typeof keywordItem.aiRelevance.score === 'number' && (
            <p className='text-xs text-slate-400'>
              ...
              <span className='font-semibold text-teal-400'>
                {keywordItem.aiRelevance.score}/5
              </span>
            </p>
          )}
          ...
          {keywordItem.aiRelevance.error && (
            <p className='text-xs text-amber-400'>
              <strong className='font-semibold'>AI Note:</strong>{' '}
              {keywordItem.aiRelevance.error}
            </p>
          )}
        </div>
