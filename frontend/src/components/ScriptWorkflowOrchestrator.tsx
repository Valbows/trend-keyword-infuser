...
const ScriptWorkflowOrchestrator: React.FC = () => {
  ...
  const [isLoadingExistingScripts, setIsLoadingExistingScripts] =
    useState<boolean>(false);
  const [loadExistingScriptsError, setLoadExistingScriptsError] = useState<
    string | null
  >(null);
  ...
  const handleLoadScriptForEditing = useCallback(
  ...
  const handleGenerateNewScript = useCallback(async () => {
    ...
    try {
      ...
      if (data.scriptId && typeof data.script === 'string') {
        ...
        throw new Error(
          'Invalid response from script generation: ID or content missing.'
        );
      }
    } catch (error: unknown) {
  ...
  const handleSaveEditedScript = useCallback(
  ...
  return (
    <div className='space-y-8 p-4 md:p-8 max-w-4xl mx-auto bg-slate-900 text-slate-100 rounded-xl shadow-2xl'>
      <div className='generation-section space-y-4 bg-slate-800 p-6 rounded-lg shadow-lg'>
        <h2 className='text-2xl font-semibold text-sky-400 border-b border-slate-700 pb-2'>
          Generate New Script
        </h2>
        <div>
          <label
